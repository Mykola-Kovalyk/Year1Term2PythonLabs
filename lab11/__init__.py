...
def find_by_date(archive_path: str, file_path: str, date_begin: datetime, date_end: datetime) -> list[str]:
    ...
    with zipfile.ZipFile(archive_path, 'r') as archive:
        ...
        with archive.open(file_path, 'r') as file:
            ...
            for i in range(len(lines)):
                ...
                try:
                    if not re.search('NASA', line):
                        continue

                    occurrence = re.search(r"\[[^\]]+.", line)
                    if not occurrence:
                        continue

                    date_as_str = occurrence.group()[1:-7]
